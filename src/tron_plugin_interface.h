...
typedef enum tron_plugin_interface_version_e {
    ...
    TRON_PLUGIN_INTERFACE_VERSION_6 = 6,
    TRON_PLUGIN_INTERFACE_VERSION_LATEST = 7,
...
typedef enum tron_plugin_msg_e {
    ...
    // Special request: the TRON app is checking if we are installed on the
    // device
 ...
 * Parameters associated with the requests the TRON application can ask the
...
The TRON application will call the plugin using this handle upon reception
...
typedef struct tronPluginInitContract_s {
    // ===== READ ONLY ===== //
    // INPUT. Used to check that `TRON_PLUGIN_INTERFACE_VERSION_LATEST` is
    // correct.
    ...
    // INPUT. Size of context allocated by the TRON application, must be
    // equal to PLUGIN_CONTEXT_SIZE.
    size_t pluginContextLength;

    // INPUT. Selector of the smart contract that TRON application is asking
    ...
    // - TRON_PLUGIN_RESULT_FALLBACK : if the signing logic should fallback to
    // the generic one
...
The TRON application will call the plugin repeatidly using this handle to
...
- Report an error to the TRON application if the smart contract has an
issue.
...
The TRON application will call this handle when the entire smart contract
...
- Report an error to the TRON application if the smart contract has an
issue.
- Request information about a TRC20 token if needed (2 max).
- Set the UI layout to SIMPLIFIED (called `UI_AMOUNT_ADDRESS`) or CUSTOM (called
`TRON_UI_TYPE_GENERIC`)
    - CUSTOM: Inform the TRON application of the number of screens needed to
display the smart contract.
    - SIMPLIFIED: Inform the TRON application of the amount and address to
display.
...
typedef struct tronPluginFinalize_s {
    ...
    // pluginContext containing the address of an TRC20 token. In this case
    // TRON will call the plugin with handle_provide_token() with the
    // requested TRC20 token information. The TRON application must be made
    // aware of TRC20 token information first using 'PROVIDE TRC 20 TOKEN
    ...
    // can set the value and TRON will modify the pointed value.
    ...
    union {
        ...
        // needed to display the smart contract in TRON_UI_TYPE_GENERIC case.
...
The TRON application will call this handle when the plugin has requested
TRC20 token information in `handle_finalize`

The plugin needs to perform the following actions:

- Check that provided token matches the one requested.
- Increase the number of screens needed for the display if needed.
- Report an error to the TRON application if the display must be aborted.

!!! warning
    The TRON application can only provide to the plugin information about
    TRC20 token it knows about.<br/>
    You need to provide the TRC20 token information to the TRON application
    using `PROVIDE TRC 20 TOKEN INFORMATION` APDU.
...
typedef struct tronPluginProvideInfo_s {
    ...
    // INPUT. TRC20 token information as requested by tokenLookup1 in
...
The TRON application will call this handle to query the contract name and
version to display.
...
The TRON application will call this handle for each screen to display if a
