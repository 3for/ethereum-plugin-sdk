# TRON plugin documentation

This documentation will help you design your own TRON plugin.

## What is a plugin?

A plugin is a lightweight application that only performs smart contract parsing and leverages the TRON application for the rest.

Your plugin will allow your users to clear-sign your smart contracts when using a Ledger device.

Plugins are designed to work hand-in-hand with the TRON application:

- The plugin handle the smart contract parsing
- The TRON application handles everything else (APDU reception, signature, screen management, bluetooth, etc)

Upon your smart contract reception, the TRON application will call your plugin to query the elements to display and your plugin will be responsible for:

- Extracting the relevant information from the smart contract.
- Replying the string to be displayed back to the TRON App.

## Coding my own plugin

### Use the TRON Plugin SDK

Use the Ledger developed [TRON Plugin SDK](https://github.com/3for/tron-plugin-sdk).<br/>
...
- `tron-plugin-sdk/`: This repository contains the interface between the TRON app and your plugin, as well as a lot of utility functions.
- `fuzzing/`: the [Fuzzer](test_framework/fuzzing.md) folder.
- `glyphs/`: The icon displayed by the TRON application when using the plugin on a Touchscreen device.
